...
word P42Display::Config( byte channel )
{
  ...
  word ChipID = 0;
	...
	ChipID = SPIReadRegister16( ReadDeviceID, false );
	if ((ChipID & 0xff00) == 0x2B00) {
	...
	#ifdef SERIAL_DEBUG
		Serial.println(F("N320: set line index to protoline 0"));
	#endif
		...
		// set all line indexes to point to protoline 0
	#ifdef SERIAL_DEBUG
		Serial.println(F("N426: set line index to protoline 0"));
	#endif
		for ( i=0; i <= TOTAL_LINES-1; i++) {
			SPIWriteByte (channel, INDEX_START_BYTES + i*3    , 0x00, false);
			SPIWriteByte (channel, INDEX_START_BYTES + i*3 + 1, PROTOLINE_WORD_ADDRESS(0), false);
...
//		SPIWriteRegister40 (WriteBlockMoveControl1, 0x0000, 0x0000, BMVC_PYF | BMVC_DACC, false );
		// set all line indexes to point to protoline 0
	#ifdef SERIAL_DEBUG
		Serial.println(F("P300: set line index to protoline 0"));
	#endif
...
// For testing purposes, make some interesting pattern to proto 0
	...
	for (i=1; i<=50; i++) {
		SPIWriteWord (channel, (u_int16)w++, 0x797F+i*0x1300, false); //"Proto-maximum" green level + color carrier wave
	}
	// To make red+blue strip
	#define RED_BIT 0x0400
	#define BLUE_BIT 0x0800
	SPIWriteWord (channel, PROTOLINE_WORD_ADDRESS(0) + 126, 0x7380 + RED_BIT, false);
	SPIWriteWord (channel, PROTOLINE_WORD_ADDRESS(0) + 130, 0x7380 + RED_BIT + BLUE_BIT, false);
	SPIWriteWord (channel, PROTOLINE_WORD_ADDRESS(0) + 134, 0x7380 + BLUE_BIT, false);
	...
	SPIWriteWord (channel, PROTOLINE_WORD_ADDRESS(0) + 156, 0xf54f+BLANK_LEVEL, false);
	SPIWriteWord (channel, PROTOLINE_WORD_ADDRESS(0) + 157, 0xf54f+BLANK_LEVEL, false);
	SPIWriteWord (channel, PROTOLINE_WORD_ADDRESS(0) + 158, 0xf54f+BLANK_LEVEL, false);
	// Orangish color to the end of proto line
	w = PROTOLINE_WORD_ADDRESS(0)+FRPORCH-1; 
	for (i=0; i<=90; i++) {
		SPIWriteWord (channel, (u_int16)w--, (WHITE_LEVEL-0x30)|0xc400, false);
	}
#endif	
...
//		SPIWriteRegister40 (WriteBlockMoveControl1, 0x0000, 0x0000, BMVC_PYF | BMVC_DACC, false );
		// set all line indexes to point to protoline 0
	#ifdef SERIAL_DEBUG
		Serial.println(F("P500: set line index to protoline 0"));
	#endif
		for ( i=0; i < TOTAL_LINES; i++) {
			SPIWriteByte (channel, INDEX_START_BYTES + i*3    , 0x00, false);
			SPIWriteByte (channel, INDEX_START_BYTES + i*3 + 1, PROTOLINE_WORD_ADDRESS(0), false);
...
// Set a pixel with the given value interpreted as VUY
// color value is a word lower bits are used for lo colour palettes
// coordinates are from (0,0) to (XPIXELS-1,YPIXELS-1)
void P42Display::SetYUVLine  (byte channel, word x, word y, word length, byte *LineArray ) {

	u_int32 address;
  u_int16 i;
	
	if ((x+length-1 <= XPIXELS-1) && (y-1 <= YPIXELS-1)) {
		address = PICLINE_BYTE_ADDRESS(y) + x;

		// SPIWriteByte (channel, address, LineArray[0], false);
  	address = address + (channel * ch_block);
  	// take the SS pin low to select the chip:
  	digitalWrite(slaveSelectPin,LOW);
  	// send in the address and return value via SPI:
    ...
    for (i=0; i < length; i++) {
      SPI.transfer( byte ( LineArray[i] & 0xFF ));
    }
	digitalWrite(slaveSelectPin,HIGH); 
	}
  else {
		Serial.println (F("SetYUVLine failed."));
		Serial.print (F("x-coord: "));
		Serial.print (x+length-1 );
		Serial.print (F(" x_max: "));
		Serial.println (XPIXELS-1);
		Serial.print (F("y-coord: "));
		Serial.print (y-1 );
		Serial.print (F(" y_max: "));
		Serial.println (YPIXELS-1);
...
//Serial.print(".");
		...
		if ( counter >= PixelOffset ) {
          ...
          pixelline8bit[0] = inbyte;
					digitalWrite(MemSelectPin,LOW);
					SPI.transfer( 0x03 );									// read data
					SPI.transfer( byte(((mem_location + counter+1) >>16) & 0xFF) );	// Flash start address of BMP pixel field
					SPI.transfer( byte(((mem_location + counter+1) >> 8) & 0xFF) );
					SPI.transfer( byte( (mem_location + counter+1)       & 0xFF) );
          for ( i=1; i<Width; i++ ) {
            pixelline8bit[i] = SPI.transfer( 0x00 );
          }
					...
					// Serial.print(mem_location+counter+1, HEX);

/*          for (i=0;i<Width;i++) {
  					// Serial.print(pixelline8bit[i], HEX);
  					// Serial.print(".");
  					SetYUVPixel ( channel, x + (counter-PixelOffset+i)%Width , y+Height - (counter-PixelOffset+i)/Width, pixelline8bit[i]); //
            // counter++; 
          }
*/
          SetYUVLine  ( channel, x + (counter-PixelOffset)%Width, y+Height - (counter-PixelOffset)/Width-1, Width,  &pixelline8bit[0] );

          counter = counter + Width - 1;  // to offset the ++ at the end of the switch statement
  					// Serial.println("<");
					...
					// pixelvalue = (red);		// translate RGB into 12 value grey scale
					// SetYUVPixel ( channel, x + (counter-PixelOffset)%Width , y+Height - (counter-PixelOffset)/Width, pixelvalue); // 
